...
import { getBlogList, getCategoryBySlug } from '@/payload/queries'

export const revalidate = 3600

export async function generateStaticParams() {
  // ponytail: no category-slug query provided; [] renders on-demand via ISR. Add a getPayloadClient().find over categories if pre-render is required.
  return []
}
...
export default async function BlogCategoryPage({ params }: Props) {
  ...
  try {
    ...
    if (category) {
      const res = await getBlogList('id', slug)
      posts = res.posts
    }
  } catch {}
