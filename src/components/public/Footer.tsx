...
export function Footer({ locale = 'id' }: FooterProps) {
  return (
    <footer className="relative" style={{ background: '#080512' }}>
      {/* Gradient top border */}
      <div className="h-px w-full" style={{ background: 'linear-gradient(90deg, transparent, #7C3AED80, #DB277780, transparent)' }} />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            ...
            <div className="flex gap-3">
              <a
                ...
                className="h-9 w-9 rounded-xl border border-[var(--border-default)] bg-[var(--bg-elevated)] flex items-center justify-center text-[var(--text-muted)] hover:text-brand-pink hover:border-brand-pink/30 transition-colors"
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="h-9 w-9 rounded-xl border border-[var(--border-default)] bg-[var(--bg-elevated)] flex items-center justify-center text-[var(--text-muted)] hover:text-emerald-400 hover:border-emerald-400/30 transition-colors"
                aria-label="WhatsApp"
              >
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              ...
              <li>
                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--text-secondary)] hover:text-emerald-400 transition-colors flex items-center gap-2">
            ...
            <div className="mt-4">
              ...
              <ul className="space-y-2">
                {BLOG_CATEGORIES.slice(0, 3).map((cat) => (
                  <li key={cat.href}>
                    <Link href={cat.href} className="text-xs text-[var(--text-muted)] hover:text-brand-violet transition-colors">
                      {cat.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-12 pt-6 border-t border-[var(--border-default)] flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <button
            ...
            className="h-9 w-9 rounded-xl border border-[var(--border-default)] bg-[var(--bg-elevated)] flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:border-brand-violet/50 transition-all"
