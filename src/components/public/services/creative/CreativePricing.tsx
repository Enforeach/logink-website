...
import { CREATIVE_DELIVERABLES, CREATIVE_ADDONS, CREATIVE_ADDONS_EN, CREATIVE_ENGAGEMENT_TYPES, CREATIVE_ENGAGEMENT_TYPES_EN } from './data'
...
function Checkbox({
  ...
  return (
    <button
      ...
      className="h-5 w-5 flex-shrink-0 rounded border-2 flex items-center justify-center transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-crimson focus-visible:ring-offset-2"
      style={
        checked
          ? { background: accentColor, borderColor: accentColor }
          : { borderColor: 'var(--border-hover)', background: '#fff' }
      }
    >
      <AnimatePresence>
        {checked && (
...
const PRICING_COPY = {
  ...
  en: {
    heading: 'Every project is unique. So is the price.',
    sub: 'Creative work doesn\'t fit in a standard box. Tell us what you need, and we\'ll build a custom scope and quote within 24 hours.',
    whatNeed: 'What do you need?',
    addonsLabel: 'Available Add-ons',
    engagementLabel: 'Engagement Type',
    briefLabel: 'Your creative brief',
    noSelect: 'Select deliverables above to build your creative brief',
    ctaQuote: 'Get a Custom Quote →',
    ctaWa: 'Chat on WhatsApp',
    waPrefix: 'Hi Logink! I\'m interested in Creative Services:\n',
    waType: 'Type',
    contactBase: '/en/contact',
  },
}

export function CreativePricing({ locale = 'id' }: { locale?: 'id' | 'en' }) {
  const ref = useRef<HTMLDivElement>(null)
  const inView = useInView(ref, { once: true, margin: '-80px' })
  const copy = PRICING_COPY[locale]
  const addonsData = locale === 'en' ? CREATIVE_ADDONS_EN : CREATIVE_ADDONS
  const engagementData = locale === 'en' ? CREATIVE_ENGAGEMENT_TYPES_EN : CREATIVE_ENGAGEMENT_TYPES
  ...
  const selectedAddons = (addonsData as typeof CREATIVE_ADDONS).filter(a => addons[a.id]).map(a => a.name.split(' & ')[0])
  ...
  const whatsappMsg = encodeURIComponent(
    ...
    (engagement ? `${copy.waType}: ${(engagementData as typeof CREATIVE_ENGAGEMENT_TYPES).find(e => e.id === engagement)?.name}` : '')
  )
  ...
  return (
    <section
      ...
      style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(245,166,35,0.07) 0%, transparent 60%), var(--bg-primary)' }}
    >
      <div className="max-w-4xl mx-auto">
        ...
        <div className="rounded-2xl border border-[var(--border-default)] bg-white p-8 space-y-8 shadow-card">

          {/* Deliverable categories */}
          <div>
            <p className="font-display text-base font-bold text-[var(--text-primary)] mb-6">{copy.whatNeed}</p>
          ...
          <div className="border-t border-[var(--border-default)] pt-6">
            <p className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-4">{copy.addonsLabel}</p>
            <div className="space-y-2">
              {(addonsData as typeof CREATIVE_ADDONS).map(addon => (
          ...
          {/* Engagement type */}
          <div className="border-t border-[var(--border-default)] pt-6">
            <p className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-4">{copy.engagementLabel}</p>
            <div className="space-y-2">
              {(engagementData as typeof CREATIVE_ENGAGEMENT_TYPES).map(type => (
                <label key={type.id} className="flex items-start gap-3 cursor-pointer group py-1.5">
                  <button
                    onClick={() => setEngagement(type.id)}
                    role="radio"
                    aria-checked={engagement === type.id}
                    className="h-5 w-5 flex-shrink-0 rounded-full border-2 flex items-center justify-center transition-all mt-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-crimson focus-visible:ring-offset-2"
                    style={
                      engagement === type.id
          ...
          <AnimatePresence>
            {hasAny && (
              <motion.div
                ...
                className="rounded-xl border border-[var(--border-default)] p-6 space-y-4"
                style={{ background: 'var(--bg-tint-peach)' }}
              >
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-base" aria-hidden>📋</span>
                  <span className="font-display text-sm font-bold text-[var(--text-primary)]">{copy.briefLabel}</span>
                </div>

                <div className="space-y-1.5 text-sm">
                  {selectedDesign.length > 0 && (
                    <div><span className="font-semibold" style={{ color: '#B45309' }}>Design: </span>
                      <span className="text-[var(--text-secondary)]">{selectedDesign.join(', ')}</span></div>
                  )}
                  {selectedVideo.length > 0 && (
                    <div><span className="font-semibold text-brand-crimson">Video: </span>
                      <span className="text-[var(--text-secondary)]">{selectedVideo.join(', ')}</span></div>
                  )}
                  {selectedCopy.length > 0 && (
                    <div><span className="font-semibold" style={{ color: '#9333EA' }}>Copy: </span>
                  ...
                  {engagement && (
                    <div className="pt-1 border-t border-[var(--border-default)] text-[var(--text-muted)] text-xs">
                      {copy.waType}: {(engagementData as typeof CREATIVE_ENGAGEMENT_TYPES).find(e => e.id === engagement)?.name}
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-3 flex-wrap pt-2">
                  <a
                    href={`${copy.contactBase}?${quoteParams}`}
                    className="gradient-bg px-6 py-3 rounded-full text-sm font-semibold text-white shadow-cta hover:scale-[1.02] hover:brightness-105 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-crimson focus-visible:ring-offset-2"
                  >
                    {copy.ctaQuote}
                  </a>
                  <a
