...
// Server-side wrapper that fetches data
...
export async function generateCaseStudyMetadata(slug: string, locale: Locale): Promise<Metadata> {
  ...
  if (!cs) return {}
  const title = locale === 'en' ? (cs.seoTitleEn || cs.titleEn || cs.titleId || cs.title) : (cs.seoTitleId || cs.titleId || cs.title)
  const description = locale === 'en' ? (cs.seoDescEn || cs.summaryEn || cs.summaryId) : (cs.seoDescId || cs.summaryId)
  const path = locale === 'en' ? `/en/portfolio/${slug}` : `/portfolio/${slug}`
  return buildMetadata({ title: (title || cs.title) as string, description: (description || `${cs.clientName} — ${cs.industry}`) as string, path })
}
...
export async function CaseStudyPage({ slug, locale }: { slug: string; locale: Locale }) {
  ...
  const [relatedCases, allServices] = await Promise.all([
    getRelated(cs),
    prisma.service.findMany({ where: { isActive: true }, select: { id: true, name: true, slug: true, color: true, icon: true } }),
  ])
